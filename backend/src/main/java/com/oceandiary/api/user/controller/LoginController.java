...
public class LoginController {
    ...
    public LoginResponse loginByKakao(LoginRequest request, HttpSession session) {
        log.info("code = {}", request.getCode());
        return loginService.naverLogin(request, session);
    }

    @PostMapping("/*/signup")
    public JoinResponse joinByKakao(@RequestBody @Valid JoinRequest request) {
        return loginService.join(request);
    }
}
